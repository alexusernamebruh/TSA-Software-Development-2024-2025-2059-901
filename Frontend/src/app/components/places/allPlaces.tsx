...
export default function AllPlaces() {
  ...
  const getPlaces = async () => {
    const { data: response } = await a.get(`/places/`);
    setPlaces(response);
    if (!currentPlace) {
  ...
  const hasRated = () => {
    if (currentPlace?.ratings) {
      const rating = currentPlace.ratings.find((v) => v.id === userId);
      if (rating) {
        return true;
      }
    }
                ...
                {currentPlace && currentPlace.ratingsAverage !== -1 ? (
                  <div>
                    <Rating
                      ...
                      readOnly
                    />
                    {!hasRated() && currentPlace?.ownerId !== userId && (
                      <div
                        onClick={() => setShowCreateRatingModal(true)}
                        className='mt-3 text-white font-semibold bg-blue-500 hover:cursor-pointer hover:bg-blue-600 w-fit h-fit rounded-md px-3 py-2'
                      >
                        Create a rating
                      </div>
                    )}
                  ...
                  <div className='text-gray-600 text-sm'>
                    ...
                    {!hasRated() && currentPlace?.ownerId !== userId && (
                      <div
                        onClick={() => setShowCreateRatingModal(true)}
                        className='mt-1 text-white font-semibold bg-blue-500 hover:cursor-pointer hover:bg-blue-600 w-fit h-fit rounded-md px-3 py-2'
                      >
